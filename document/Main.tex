...
% --- Section 3: Data Description & EDA ---
...
\begin{itemize}
    \item \textbf{Class Distribution:} The dataset exhibits a relatively balanced patient-level distribution (approximately 91 Normal and 98 Abnormal patients), mitigating the need for aggressive synthetic oversampling techniques.
...
% Uncomment and add your image here
\begin{figure}[H]
    \centering
...
% --- Section 6: Resolution & Evaluation ---
...
The metrics display a highly calibrated model. The balanced sensitivity and specificity ensure the system acts as a reliable filter without overwhelming cardiologists with false positives.
...
To truly resolve the problem, the model was packaged into an interactive clinical dashboard using \texttt{Streamlit}. Physicians can upload multiple audio files simultaneously. The system processes the audio, applies the mathematical scaling to clinical inputs, and generates a fully formatted PDF report detailing the AI's confidence levels and plotting the exact spectrogram windows that triggered anomalies. 
...
To test the model, some data from the test set was taken and given to the model in order to verify its results:
...
\bibitem{lam2005stethoscope}
Lam, C. S., \& Kumar, S. (2005). The stethoscope: An outdated tool or an essential clinical companion? \textit{The American Journal of Medicine}, 118(12), 1435--1436.
...
Reyna, M. A., et al. (2022). Heart murmur detection from phonocardiogram recordings: The George B. Moody PhysioNet Challenge 2022. \textit{Computing in Cardiology (CinC)}, 49, 1--4.

\bibitem{tan2019efficientnet}
Tan, M., \& Le, Q. (2019). EfficientNet: Rethinking model scaling for convolutional neural networks. \textit{International Conference on Machine Learning (ICML)}, 6105--6114.
